...
handleShowInstructions=(cx, cy)=>{
  ...
  if(cx >= INSTRUCTIONS_BUTTON_POS.x && cx <= INSTRUCTIONS_BUTTON_POS.x + SUBMIT_SIZE.x &&
    cy >= INSTRUCTIONS_BUTTON_POS.y && cy <= INSTRUCTIONS_BUTTON_POS.y+SUBMIT_SIZE.y){
      ...
      ctx.fillStyle = "#847b7e";
      ctx.fillText("The game ends when you find ten sets.", INSTRUCTIONS_POS.x+10, INSTRUCTIONS_POS.y+470 );

        // The variations of each feature are as follows
  }
...
showNumberOfMoves=()=>{
  ...
  ctx.fillStyle = "white";
  ctx.font = "19px Georgia";
  ctx.fillText(`Valid Moves: ${game.findAllValidSelections(deck.faceUpCards).length}`, SHOW_VALID_POS.x+13, SHOW_VALID_POS.y+22);
}
...
showScore=()=>{
  ...
  ctx.fillStyle = "white";
  ctx.font = "19px Georgia";
  ctx.fillText(`Score: ${score}`, SCORE_POS.x+42, SCORE_POS.y+22);
}
