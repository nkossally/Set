...
let canvasBackground = document.getElementById("canvas-3");
let ctxBackground = canvasBackground.getContext("2d");
  ...
  const SHOW_MOVE_POS = {x: DEAL_THREE_MORE_POS.x, y: DEAL_THREE_MORE_POS.y+SUBMIT_SIZE.y+50};
  const SHOW_VALID_POS = {x: SHOW_MOVE_POS.x, y: SHOW_MOVE_POS.y+SUBMIT_SIZE.y+50};
...
newGame=()=>{
  ...
  ctx.rect(IN_SET.x/2, IN_SET.y-15, 
    OUT_OF_PLAY_POS.x+(1/3)*CARD_SIZE.x*9+(1/3)*CARD_MARGIN.x*6+IN_SET.x/2+20, 3*CARD_SIZE.y+2*CARD_MARGIN.y+40 );
  ctx.stroke();
...
showNumberOfMoves=()=>{
  ...
  ctx.fillText(`${game.findAllValidSelections(deck.faceUpCards).length}`, SHOW_VALID_POS.x+68, SHOW_VALID_POS.y+19);
  // ctx.fillText("Show Move", SHOW_MOVE_POS.x, SHOW_MOVE_POS.y+30);
}
...
drawOutOfPlayCard=()=>{
  ...
  let column = Math.floor(idx/27)* ((CARD_SIZE.x+CARD_MARGIN.x)*scale*3+5);
  let pos = {x: OUT_OF_PLAY_POS.x+(idx%3)*(CARD_SIZE.x+CARD_MARGIN.x)*scale +column, y: OUT_OF_PLAY_POS.y+(Math.floor(idx/3)%9)*(CARD_SIZE.y+CARD_MARGIN.y)*scale}
