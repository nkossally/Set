...
let ctxBackground = canvasBackground.getContext("2d");
  ...
  const SCORE_POS = {x: IN_SET.x+10+5*(CARD_SIZE.x+CARD_MARGIN.x), y: IN_SET.y};
...
handleSelect=(cx, cy)=>{
  for (let i = 0; i < 15; i++) {
    const pos = {x: IN_SET.x+10+Math.floor(i/3)*(CARD_SIZE.x+CARD_MARGIN.x), y: IN_SET.y+(i%3)*(CARD_SIZE.y+CARD_MARGIN.y)};
    if(cx >= pos.x && cx <= pos.x + CARD_SIZE.x &&
      ...
      if (deck.selected.length > 3) {
        ...
        const oldPos = {x: IN_SET.x+10+Math.floor(idx/3)*(CARD_SIZE.x+CARD_MARGIN.x), y: IN_SET.y+(idx%3)*(CARD_SIZE.y+CARD_MARGIN.y)};
    ...
    if(deck.selected.length ===3){
      if(game.isValidSelection(deck.selected)){
        deck.selected.forEach(card=> {
          ...
          const pos = {x: IN_SET.x+10+Math.floor(idx/3)*(CARD_SIZE.x+CARD_MARGIN.x), y: IN_SET.y+(idx%3)*(CARD_SIZE.y+CARD_MARGIN.y)};
