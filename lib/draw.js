...
class Draw {
  ...
  drawInstructions(ctx, CARD_MARGIN, INSTRUCTIONS_POS, CLOSE_INSTRUCTIONS_POS, CLOSE_INSTRUCTIONS_SIZE, INSTRUCTIONS_BUTTON_POS, INSTRUCTIONS_SIZE, SUBMIT_SIZE ){
    this.drawButton(ctx, INSTRUCTIONS_BUTTON_POS, "Instructions", {x: 23, y: 21}, "gray", SUBMIT_SIZE);
  ...
  clearCard(ctx, pos, scale){
    this.drawRoundedRec(ctx, {x: pos.x-1, y: pos.y-1}, scale*CARD_SIZE.x+2, scale*CARD_SIZE.y+2, scale*CARD_SIZE.x/10, "#BDF3FF");
  }
  ...
  drawRectangles(ctx, card, pos, scale){
    if(card.number === 1){
      this.drawRectangle(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*11/6}, scale)
    } else if (card.number === 2){
      this.drawRectangle(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*7/6}, scale)
      this.drawRectangle(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*15/6}, scale)
    } else {
      this.drawRectangle(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale/2}, scale)
      this.drawRectangle(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*11/6}, scale)
      this.drawRectangle(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*19/6}, scale)
    }
  }
  
  drawOvals(ctx, card, pos, scale){
    if(card.number === 1){
      this.drawOval(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*11/6}, scale)
    } else if (card.number === 2){
      this.drawOval(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*7/6}, scale)
      this.drawOval(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*15/6}, scale)
    } else {
      this.drawOval(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale/2}, scale)
      this.drawOval(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*11/6}, scale)
      this.drawOval(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*19/6}, scale)
    }
  }
  
  drawDiamonds(ctx, card, pos, scale){
    if(card.number === 1){
      this.drawDiamond(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*11/6}, scale)
    } else if (card.number === 2){
      this.drawDiamond(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*7/6}, scale)
      this.drawDiamond(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*15/6}, scale)
    } else {
      this.drawDiamond(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale/2}, scale)
      this.drawDiamond(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*11/6}, scale)
      this.drawDiamond(ctx, card, {x: pos.x+SYMBOL_SIZE*scale*2/3, y: pos.y+SYMBOL_SIZE*scale*19/6}, scale)
    }
  }
  ...
  animateDrawCard(ctx, pos, startScale, endScale, color, card, direction){
    ...
    if(startScale !== endScale){
      ...
      requestAnimationFrame(function(){that.animateDrawCard(ctx, pos, startScale, endScale, color, card, direction)});
    }
  }
